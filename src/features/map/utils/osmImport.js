...
// Fetch general OSM features (golf, natural=water/sand) within a specific bounding box with optional filters
export async function fetchOSMFeaturesInBbox(minLat, minLon, maxLat, maxLon, filters, options = {}) {
  const bboxQuery = `(${minLat}, ${minLon}, ${maxLat}, ${maxLon})`

  const clauses = []
  if (!filters) {
    clauses.push(`nwr["golf"]${bboxQuery};`)
    clauses.push(`nwr["natural"="water"]${bboxQuery};`)
    clauses.push(`nwr["natural"="sand"]${bboxQuery};`)
  } else {
    if (filters.tees) clauses.push(`nwr["golf"="tee"]${bboxQuery};`)
    if (filters.greens) clauses.push(`nwr["golf"="green"]${bboxQuery};`)
    if (filters.fairways) clauses.push(`nwr["golf"="fairway"]${bboxQuery};`)
    if (filters.bunkers) {
      clauses.push(`nwr["golf"="bunker"]${bboxQuery};`)
      clauses.push(`nwr["natural"="sand"]${bboxQuery};`)
    }
    if (filters.water) {
      clauses.push(`nwr["golf"="water_hazard"]${bboxQuery};`)
      clauses.push(`nwr["natural"="water"]${bboxQuery};`)
    }
    if (filters.rough) clauses.push(`nwr["golf"="rough"]${bboxQuery};`)
  }
