...
export default function DigitalBagPanel({ userId }) {
  ...
  const loadClubs = useCallback(async () => {
    ...
    const { data, error: qErr } = await supabase
      .from('clubs')
      .select(
        'id, name, short_name, club_type, is_putter, sort_order, carry_distance, total_distance, created_at',
      )
      .eq('user_id', userId)
      .eq('is_active', true)
  ...
  useEffect(() => {
    ...
    loadClubs().then((rows) => {
      if (cancelled) return
      setAddSortOrder(String(nextSortAfterRows(rows)))
    })
    return () => {
      cancelled = true
    }
  ...
  return (
    <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 mt-8">
      ...
      {!loading && (
        <>
          <form onSubmit={handleAdd} className="mb-6 rounded-lg border border-slate-600/80 bg-slate-900/50 p-4">
          ...
          {clubs.length === 0 ? (
            <p className="text-slate-500 text-sm">No clubs yet. Use the form above to build your bag.</p>
          ) : (
